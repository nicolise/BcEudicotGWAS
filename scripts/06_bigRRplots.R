...
HEM.plotdata <- read.csv("data/GWAS_files/04_bigRRoutput/Sp_LesionSize_MAF20.HEM.PlotFormat.csv")
...
HEM.thresh <- read.csv("data/GWAS_files/04_bigRRoutput/Sp_LesionSize_MAF20.HEM.Thresh.csv")
...
for (y in 3:14){
jpeg(paste("plots/MultiPlot/Sp_LesionSize_MAF20_lowTR_", names(HEM.plotdata[y]), ".ManhattanPlot.jpg", sep=""), width=8, height=4, units='in', res=600)
...
ggplot(HEM.plotdata, aes(x=Index, y=abs(HEM.plotdata[y])))+
  ...
  labs(list(y="SNP Effect Estimate", x="Chromosome position", title=paste("Lesion Size on ", names(HEM.plotdata[y]))))+
  guides(col = guide_legend(nrow = 8, title="Chromosome"))+
  geom_hline(yintercept=get(paste("TH999_", names(HEM.plotdata[y]), sep=""))) +
  geom_text(aes(0,get(paste("TH999_", names(HEM.plotdata[y]), sep="")), label =
