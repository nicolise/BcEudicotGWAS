...
HEM.plotdata <- read.csv("data/BcAtGWAS/04_bigRRoutput/0523outs/BcAtGWAS.HEM.PlotFormat.csv")
...
HEM.thresh <- read.csv("04_bigRRoutput/0523outs/BcAtGWAS.HEM.Thresh.csv")
...
#without the loop
for (y in 4:15){#[14]
jpeg(paste("plots/BcAtGWAS/052316/BcAt_MAF20_lowTR_", names(HEM.plotdata[14]), ".ManhattanPlot.jpg", sep=""), width=8, height=4, units='in', res=600)
...
jpeg(paste("plots/BcAtGWAS/052316/BcAt_MAF20_highTR_", names(HEM.plotdata[14]), ".ManhattanPlot.jpg", sep=""), width=8, height=4, units='in', res=600)
ggplot(HEM.plotdata, aes(x=Index, y=abs(HEM.plotdata[14])))+
  theme_bw()+
  geom_point(aes(color = factor(Chrom)))+
  labs(list(y="SNP Effect Estimate", x="Chromosome position", title=paste("Lesion Size on ", names(HEM.plotdata[14]))))+
